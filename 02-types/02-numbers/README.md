...
1. Write a function with the below signature that receives two `float64` arguments `x` and `y` and returns the geometric mean of `x` and `y` rounded to the nearest integer (so the result for `x=1.2` and `y=2.4` would be `round(sqrt(1.2*2.4)=round(1.69)=2`):
